...
function devs()
   ...
   for line in devs do
      ...
      local mc, ac, f, f1, f2, f3, f4, t1, t2, t3, t4, dh, nmw, cacn1, cacn2, cacn3, cacn4, cacs1, cacs2, cacs3, cacs4, cv = line:match("miner_count=(%d+),asic_count=(%d+),.*,frequency=(%d+),.*,fan1=(%d+),fan2=(%d+),fan3=(%d+),fan4=(%d+),.*,temp1=(.*),temp2=(.*),temp3=(.*),temp4=(.*),.*,Device Hardware%%=([%d%.]+),no_matching_work=(%d+),chain_acn1=(%d+),chain_acn2=(%d+),chain_acn3=(%d+),chain_acn4=(%d+),chain_acs1=(.*),chain_acs2=(.*),chain_acs3=(.*),chain_acs4=(.*),USB Pipe=(%d+)")
      if mhs5s then
        st = status
        m5 = mhs5s
      end
...
function devs1()
   ...
   for line in devs do
      local fv = fver:match("(.*)")
      local status, mhs5s = line:match("Status=(%a+).*MHS 5s=([%d%.]+)")
      local mc, ac, f, f1, f2, f3, f4, t1, t2, t3, t4, dh, nmw, cacn1, cacn2, cacn3, cacn4, cacs1, cacs2, cacs3, cacs4, cv = line:match("miner_count=(%d+),asic_count=(%d+),.*,frequency=(%d+),.*,fan1=(%d+),fan2=(%d+),fan3=(%d+),fan4=(%d+),.*,temp1=([%-%d]+),temp2=([%-%d]+),temp3=([%-%d]+),temp4=([%-%d]+),.*,Device Hardware%%=([%d%.]+),no_matching_work=(%d+),.*,chain_acn1=(%d+),chain_acn2=(%d+),chain_acn3=(%d+),chain_acn4=(%d+),chain_acs1=(.*),chain_acs2=(.*),chain_acs3=(.*),chain_acs4=(.*),USB Pipe=(%d+)")
      if mhs5s then
        st = status
        m5 = mhs5s
      end
...
function pools()
   ...
   for line in pools do
	 ...
	 "POOL=(%d+),URL=(.*),Status=(%a+),Priority=(%d+),.*,Long Poll=(%a+),Getworks=(%d+),Accepted=(%d+),Rejected=(%d+),Discarded=(%d+),Stale=(%d+),Get Failures=(%d+),Remote Failures=(%d+),User=(.*),Last Share Time=(%d+),Diff=(.*),Diff1 Shares=(%d+),Proxy Type=.*,Proxy=.*,Difficulty Accepted=(%d+)[%.%d]+,Difficulty Rejected=(%d+)[%.%d]+,Difficulty Stale=(%d+)[%.%d]+,Last Share Difficulty=(%d+)[%.%d]+,Has Stratum=(%a+),Stratum Active=(%a+),Stratum URL=.*,Has GBT=(%a+)")
      if pi then
	 if lst == "0" then
	    lst_date = "Never"
	 else
